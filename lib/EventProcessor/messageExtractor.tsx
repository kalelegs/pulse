...
const ASSISTANT_DONE_TYPES = new Set<string>([
...
export type TMessageExtractor = {
   ...
   * state. Wired to `useSession`'s `onDisconnect` by `components/RealtimeExperience`.
   */
  reset: () => void;
};
 ...
 *
 * This is the message half of the transport pipeline; `processEvent` in `./EventProcessor` is the
 * debug-panel half, and the two share nothing. Per-role bookkeeping lives in `./assistantTurn` and
