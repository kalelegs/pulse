/**
 * Realtime model configuration shared by the server action that mints the ephemeral client secret
 * and the client-side `RealtimeSession`. They must never drift, which is why both read from here.
 */
